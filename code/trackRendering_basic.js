...
class trackRendering_basic {
    ...
    renderTrack(container, track) {
        ...
        let p1 = geometry.perpendicular(track.start, track._tmp.deg, -8);
        let p2 = geometry.perpendicular(track.start, track._tmp.deg, 8);
        let p3 = geometry.perpendicular(track.end, track._tmp.deg, 8);
        const p4 = geometry.perpendicular(track.end, track._tmp.deg, -8);
        ...
        if (!track._tmp.switches.some((p) => p.km == 0)) {
            //prellbock beim start
            p1 = geometry.perpendicular(track.start, track._tmp.deg, -6);
            p2 = geometry.perpendicular(track.start, track._tmp.deg, 6);
            shape.graphics.moveTo(p1.x, p1.y).lineTo(p2.x, p2.y);
        }

        if (!track._tmp.switches.some((p) => p.km == track._tmp.length)) {
            //prellbock beim ende
            p1 = geometry.perpendicular(track.end, track._tmp.deg, -6);
            p2 = geometry.perpendicular(track.end, track._tmp.deg, 6);
            shape.graphics.moveTo(p1.x, p1.y).lineTo(p2.x, p2.y);
        }
    ...
    renderAllSwitches() {
        switches.forEach((sw) => {
            ...
            p2 = geometry.add(sw.location, geometry.multiply(sw.t2._tmp.unit, l));
    ...
    renderSwitchUI(sw) {
        ...
        p1 = geometry.add(sw.location, geometry.multiply(sw.t1._tmp.unit, -10));
        ui_shape.graphics.moveTo(sw.location.x, sw.location.y).lineTo(p1.x, p1.y);

        p1 = geometry.add(sw.location, geometry.multiply(sw.t1._tmp.unit, 10));
        ui_shape.graphics.moveTo(sw.location.x, sw.location.y).lineTo(p1.x, p1.y);

        p1 = geometry.add(sw.location, geometry.multiply(sw.t2._tmp.unit, l));
        ui_shape.graphics.moveTo(sw.location.x, sw.location.y).lineTo(p1.x, p1.y);
        ...
        p1 = geometry.add(sw.location, geometry.multiply(sw.branch._tmp.unit, l));
        ui_shape.graphics.moveTo(sw.location.x, sw.location.y).lineTo(p1.x, p1.y);
    }

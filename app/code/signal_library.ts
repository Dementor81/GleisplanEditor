...
// ES6 Module imports
import { VisualElement, TextElement } from './visualElement.ts';
...
export function initSignals(signalTemplatesRef: any) {
   ...
   t.scale = 0.15;
   (t as any).previewsize = 20;
   t.distance_from_track = 5;
   (t as any).checkSignalDependency = checkSignalDependencyFunction4HV;
   ...
   //signal: ist das signal, dessen Stellung wir gerade setzen
   //hp: ist das signal, dessen Stellung wir vorsignalisieren wollen
   (t as any).checkSignalDependency = function (signal: any, hp: any) {
      //make sure we only handle main signals
      if (!hp.check("HPsig") || !signal.check("VRsig")) return;
      let stop_propagation = false;
   ...
   (t as any).checkSignalDependency = signalTemplates.ks.checkSignalDependency;
   ...
   (signalTemplates.ne4 as any).previewsize = 10;
   ...
   (signalTemplates.ne2 as any).previewsize = 20;
   ...
   (signalTemplates.lf6 as any).createSignalCommandMenu(["geschw()"]);
   ...
   (signalTemplates.lf7 as any).createSignalCommandMenu(["geschw()"]);
   signalTemplates.lf7.scale = 0.15;
   (signalTemplates.lf7 as any).previewsize = 30;
   ...
   (signalTemplates.zs6 as any).previewsize = 30;
   ...
   (signalTemplates.zusatzSignal as any).createSignalCommandMenu([["zs6=1(Zs 6)"], "zs3()"]);
   (signalTemplates.zusatzSignal as any).previewsize = 30;
}

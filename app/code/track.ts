...
export class Track {
   ...
   static isValidTrackNodePoint(p: any, nodes: any[] | null): boolean {
   ...
   static splitTrackAtPoint(track: Track, point: Point): [Track, Track] {
    ...
    */
   static joinTrack(track1: Track, track2: Track): void {
   ...
   static splitTracksAtIntersections(): void {
      let intersection,
   ...
   static removeTrack(track: Track): void {
      ...
      Switch.updateSwitchAtPoint(track.start, startSwitch instanceof Switch ? startSwitch : undefined);
      Switch.updateSwitchAtPoint(track.end, endSwitch instanceof Switch ? endSwitch : undefined);
   ...
   static createRailNetwork(): void {
      Track.splitTracksAtIntersections();
      Track.cleanUpTracks();
      Track.createSwitches();
   }

   static initEditTrackMenu(track: Track): void {
   ...
   #_start: Point | null = null;
   #_end: Point | null = null;
   #_has_bumper: boolean = true;
   #_signals: Signal[] = [];
   ...
   #_unit: V2 | null = null;
   #_slope: number | null = null;
   #_sin: number | null = null;
   #_cos: number | null = null;
   ...
   set switches(value: (Switch | Track | null)[]) {
      this.#_switches = value;
   }

   get switchAtTheEnd(): Switch | Track | null {
      return this.#_switches[1];
   }

   set switchAtTheEnd(value: Switch | Track | null) {
      this.#_switches[1] = value;
   }

   get switchAtTheStart(): Switch | Track | null {
      return this.#_switches[0];
   }

   set switchAtTheStart(value: Switch | Track | null) {
      this.#_switches[0] = value;
   }

   constructor(start: Point | { x: number; y: number }, end: Point | { x: number; y: number }) {
   ...
   //returns the Point
   getPointFromKm(km: number): Point {
   ...
   along(point: Point, x: number): any {
      return geometry.add(point, geometry.multiply(this.unit, x));
   }

   AddSignal(signal: Signal, km?: number, above?: boolean, flipped?: boolean): void {
   ...
   static FromObject(o: any): Track {
      ...
      t.id = o.id;
      t.signals = o.signals;
      (t as any).switches_data = o.switches;
    ...
    */
   setNewStart(newStart: Point): void {
      if (!newStart) return;
